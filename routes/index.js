  ...
  if (req.session.user) {
    ...
    let cartCount=await userHelpers.getCartCount(req.session.user._id)
      productHelpers.getHomeProducts().then((products) => {
        ...
        res.render('index', { pageUser: true, uname, products,cartCount,home: true,banner})
      })
  }  
  else {
    productHelpers.getHomeProducts().then((products) => {
      res.render('index', { pageUser: true, uname: false, products,home: true,banner })
    })
  }
})
})     
...
router.get('/login', function (req, res, next) {
  ...
  if (user) {
    ...
    res.render('user/user-login', { title: 'User-Login', "loginError": req.session.loginErr, pageUser: true });
...
router.get('/shop/',async function (req, res, next) {
  ...
  console.log("-limitt-----",limitCount);

  if (req.session.user) {
    let cartCount=await userHelpers.getCartCount(req.session.user._id)
    let products=await productHelpers.getAllProductsPageWise(skipCount,limitCount)
      res.render('user/shop', { pageUser: true, uname, products,cartCount,shop:true,page,pagesCount})
  }
  else {
    productHelpers.getAllProductsPageWise(skipCount,limitCount).then((products) => { 
      res.render('user/shop', { pageUser: true, uname: false, products,shop:true,page,pagesCount })
    })
  }
})
  ...
  userHelpers.doLogin(req.body).then((response) => {
    ...
    if (response.status) {
      if (response.block) {
        loginErr = true
        loginBlock = true
        req.session.loginErr = "You are blocked, Kindly request admin to unblock"
        res.redirect('/login')
      } 
      else {
  ...
  userHelpers.verifyUserRegistration(req.body).then((response)=>{
    if(response.status){
      ...
      console.log("SendMessage");
        client.verify.v2.services('VA5759a04253cbaeee62fe53efa6b683d8')
          .verifications
      ...
      if(response.findMobile){
        res.render('user/signup',{loginError:"Mobile No. is already found!"}) 
      }
    }
  })
...
router.post('/verify-Mobile',(req,res)=>{
  ...
  if (req.session.user) {
      ...
      .then(verification_check => {
        ...
        if (verification_check.status) {
          ...
          userHelpers.addUsers(req.session.user).then((response) => {
            console.log(response);
            loginErr=true
            res.render('user/user-login', { pageUser: true, loginErr: "Registration Success, Kindly Login" });
          })
        }
        else{
    ...
    res.render('user/signup', { pageUser: true, loginErr: "Request Timed out!" })
  }
...
router.get('/list-products', function (req, res, next) {
  productHelpers.getAllProducts().then((products) => {
    res.render('admin/list-products', { pageAdmin: true, products, tabProducts:true })
  })
})
...
router.get('/add-Products', function (req, res, next) {
  userHelpers.getCategory().then((catCollect)=>{
    console.log(catCollect);
    res.render('admin/add-products', { pageAdmin: true,tabProductsAdd:true, tabProducts:true,catCollect})
  })
})
...
router.get('/edit/', async (req, res) => {
  ...
  res.render('admin/update-products', {pageAdmin:true,tabProducts:true, product, prodId });
})
...
router.get('/category',function(req,res,next){
  userHelpers.getCategory().then((catCollect)=>{
  res.render('admin/category',{pageAdmin: true,tabCategory:true,catCollect,add:true})
  })
})
...
router.get('/product-detail',async (req,res,next)=>{
  ...
  await productHelpers.ProductDetails(prodId).then((ProductInDet)=>{
    productHelpers.categoryWiseProducts(category).then((products)=>{
      res.render('user/product-details',{ pageUser: true,ProductInDet,products})  
    })
  })
})
...
router.get('/edit-category/', async (req, res) => {
  ...
  userHelpers.getCategory().then((catCollect)=>{
    res.render('admin/category', { pageAdmin:true,catId,thisCat,catCollect,tabCategory:true });
  })
})
...
router.get('/cart', async (req, res, next) => {
  ...
  if (req.session.user) {
    try {
      ...
      let cartCount = await userHelpers.getCartCount(req.session.user._id)
      console.log("-----------cart----------try:",cartCount);
      if (cartCount > 0) {
        ...
        await userHelpers.getTotalAmount(req.session.user._id).then((response)=>{
          total=Math.ceil(response.total)
          grandTotal=Math.ceil(response.grandTotal)
        })
        let offerAmount=grandTotal-total
        console.log("Total Cart Amount: ",total," ",grandTotal);
        res.render('user/cart', { pageUser: true, uname, products, total, user: req.session.user._id, cartCount,grandTotal,offerAmount })
      }
      else {
...
router.get('/place-order',async (req,res,next)=>{  
  ...
  if (req.session.user){
    userHelpers.getUserDetails(req.session.user._id).then(async (response)=>{
      ...
      {
        //console.log("----------req.cart--------",req.session.user.cart,"\n",req.session.user);
          //console.log(req.session.user._id);
          let userAddress=await userHelpers.getUserAddress(req.session.user._id)
          //console.log("User Address\n",userAddress);
          let total=await userHelpers.getTotalAmount(req.session.user._id)
          console.log("Total",total); 
          res.render('user/place-order',{pageUser:true,uname,total,userAddress})
  ...
  userHelpers.findUserMobile(mobile).then((response)=>{
    if(response.status){
      if(response.unBlkStatus)
      {
        ...
        res.render('user/otp-login',{loginError:"User is Blocked. Please ask Admin to unblock"}) 
      }
    }
    else{
      res.render('user/otp-login',{loginError:"User not found. Please Register"})
    }
  })
 })
...
router.post('/otp-submit', (req, res, next) => {
  ...
  if (mobile) {
    ...
    res.render('user/otp-login', { loginError: "Request Timed Out, Try again" })
  }
})
  ...
  if (req.session.user) {
    userHelpers.getUserDetails(req.session.user._id).then((response) => {
      ...
      userHelpers.getUserOrders(req.session.user._id).then((orders)=>
      {
        ...
        res.render('user/account', { pageUser: true, uname,userDetails,orders })
      })
    })
  }
  else {
...
router.get('/product-category-view/',(req,res)=>{
  ...
  productHelpers.categoryWiseProducts(name).then((products)=>{
    userHelpers.getCategory().then((catCollect)=>{
      res.render('admin/category-wise-products',{pageAdmin:true,tabCategory:true,products,catCollect})
    })
  })
})
...
router.post('/order-complete',async (req,res)=>{
  ...
  if(req.session.user){
    ...
    //console.log(req.body);
    let cartOrderList=await userHelpers.getOrderedProducts(req.session.user._id)
    ...
    if(paymentMode=='COD'){
      await productHelpers.addToOrders(order).then(async (response)=>{
        if(response){
          ...
          await userHelpers.removeFromCart(req.session.user._id).then((response)=>{
            if(response){
              console.log("Removed from Cart, COD Success")
      ...
      productHelpers.generateRazorpay(orderId,totalAmount)
      .then((order)=>{
  ...
  productHelpers.verifyPayment(req.body).then(async (response)=>{
    if(response.status){
      await productHelpers.addToOrders(orderArray).then(async (response)=>{
        await userHelpers.removeFromCart(req.session.user._id).then((response)=>{
          if(response){
            console.log("Removed from Cart, ONLINE placed");
            res.json({status:true})
          }
        })
      })
...
router.get('/banner-management', (req,res)=>{
  productHelpers.getBanners().then((banner)=>{
    console.log(banner);
  res.render('admin/banner-management',{pageAdmin:true,tabBanner:true,banner})
  })
})
...
router.get('/order-update/',async (req,res)=>{
  ...
  await productHelpers.changeOrderStatus(userId,orderId,productId,orderStatus).then((response)=>{
...
router.post('/update-password',async (req,res)=>{
  ...
  userHelpers.updatePassword(req.session.user._id,password).then((response)=>{
    res.json({status:true})
  })
})
...
router.get('/coupon-management',async (req,res)=>{
  ...
  await productHelpers.getAllCoupons().then((couponCollect)=>{
    console.log(req.session.admin.err);
    res.render('admin/coupon-management',{pageAdmin:true,tabCoupon:true,couponCollect,err:req.session.admin.err})
  })
  }else{
    res.redirect('/admin-login')
  }
...
router.post('/coupon-add',(req,res)=>{
  productHelpers.addCoupon(req.body).then((response)=>{
    if(response.status){
      //console.log("---------re stat",response.status);
...
router.post('/verify-coupon',(req,res)=>{
  ...
  productHelpers.verifyCoupon(couponCode).then((response)=>{
    if(response.couponDetails){
      totalAmount=(parseFloat(totalAmount))-(parseFloat(response.couponDetails.price))
      res.json({status:true,totalAmount:totalAmount,price:response.couponDetails.price})
    }
    else{
      res.json({status:false})
    }
